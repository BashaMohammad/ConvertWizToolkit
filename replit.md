...
Changelog:
- July 06, 2025 (Admin Dashboard Simplified). Replaced complex admin authentication system with simplified Firebase-based approach: Created minimal admin.html with direct Firebase v9.22.0 integration, eliminated localStorage dependency and cross-page auth state issues, added firebase-config.js for centralized configuration, implemented clean logout functionality with redirect to homepage, removed infinite scroll and positioning bugs with overflow:hidden, preserved admin access control through Firebase auth state monitoring, maintained simple UI with centered container and professional styling
