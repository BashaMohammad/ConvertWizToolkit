...
Changelog:
- July 06, 2025 (Admin Analytics Streamlined). Simplified admin dashboard to focus on live Looker Studio analytics: Removed static metric cards to eliminate mock data, beautiful gradient background (linear-gradient 120deg f6f9fc to edf2fb), enhanced container styling with rgba white background and 25px shadow, larger iframe (520px height) with rounded corners and shadows, clean typography and spacing focused on analytics display, upgraded to ConvertWiz Admin v1.2 with Live GA4 Analytics footer, maintained authentication stability with localStorage fallback system
