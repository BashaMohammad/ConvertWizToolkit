...
Changelog:
- July 06, 2025 (Admin Analytics v1.2 Enhanced). Upgraded admin dashboard with enhanced visual design and GA4 live cards: Beautiful gradient background (linear-gradient 120deg f6f9fc to edf2fb), colorful metric cards with unique gradients (users: purple-blue, country: pink-red, traffic: blue-cyan, visits: green-cyan, growth: pink-yellow), hover animations with translateY(-5px) transform effects, enhanced container styling with rgba white background and 25px shadow, larger iframe (520px height) with rounded corners and shadows, improved typography and spacing throughout, upgraded to ConvertWiz Admin v1.2 with Live GA4 Analytics footer, maintained authentication stability with localStorage fallback system
